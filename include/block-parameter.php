...
$(document).ready(function(){
    $('#blocktrackbar').trackbar({
        ...
        leftValue:
        <?php
	if ((int)$_GET["start_price"] >=1000 AND (int)$_GET["start_price"] <=40000){
 ...
 ?>,
        ...
        rightValue:       
         <?php
	if ((int)$_GET["end_price"] >=1000 AND (int)$_GET["end_price"] <=40000){
...
<?php
    ...
    if(mysql_num_rows($result) > 0){
        ...
        echo'
        <li>
        <input  '.$checked_brand.' type="checkbox" name="brand[]"  value="'.$row["id"].'" id="checkbrand'.$row["id"].'" /><label for="checkbrand'.$row["id"].'">'.$row["brand"].'</label>
        </li>';
    }
