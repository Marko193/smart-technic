...
define('myyshop', true);
  ...
  if (isset($action))
{
       if($_SESSION['accept_orders'] == '0'){
        $msgerror = 'У вас нет прав на изменение статуса заказов';
        }
        else{
   switch ($action) {
	    case 'accept':
...
{
    $status = '<span class="orange">В обработке</span>';
} 
...
else
{
 ...
 echo '
  ...
  <p class="view-order-link" >
    <a class="delete" href="view_order.php?id='.$row["order_id"].'&action=reject" >Отклонить заказ</a>  |
      <a class="orange" href="view_order.php?id='.$row["order_id"].'&action=accept">Отправить в обработку </a> | 
  <a class="purple" href="view_order.php?id='.$row["order_id"].'&action=delivery" >Доставить</a> |
  <a class="green" href="view_order.php?id='.$row["order_id"].'&action=close" >Закрыть</a> 
