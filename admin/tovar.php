...
if (isset($cat))
{
   switch ($cat) {
        ...
        $url = "cat=mobile&";
	    $cat = "WHERE product_type='mobile'";       
	    break;
	    case 'notebook':
        $cat_name = 'Ноутбуки';
        $url = "cat=notebook&";
	    $cat = "WHERE product_type='notebook'"; 
	    break; 
	    case 'notepad':
        $cat_name = 'Планшеты';
        $url = "cat=notepad&";
	    $cat = "WHERE product_type='notepad'"; 
	    break;                       
	    default:       
        $cat_name = $cat;   
        $url = "type=".clear_string($type)."&cat=".clear_string($cat)."&";
        $cat = "WHERE product_type='".clear_string($type)."' AND brand='".clear_string($cat)."'";         
	    break;
	} 
}
else
...
<?php
 ...
 $result1 = mysql_query("SELECT * FROM categories WHERE type='mobile'",$link);
...
<ul style=" margin-top: 19.5px;">
<li><a href="tovar.php?cat=notebook"><strong>Ноутбуки</strong></a></li>
<?php
 $result1 = mysql_query("SELECT * FROM categories WHERE type='notebook'",$link);
  If (mysql_num_rows($result1) > 0)
{
$row1 = mysql_fetch_array($result1);
do
...
<?php
 $result1 = mysql_query("SELECT * FROM categories WHERE type='notepad'",$link);
  If (mysql_num_rows($result1) > 0)
{
$row1 = mysql_fetch_array($result1);
do
...
<a class="green" href="edit_product.php?id='.$row["products_id"].'">Изменить</a> | <a href="tovar.php?'.$url.'id='.$row["products_id"].'&action=delete" class="delete" >Удалить</a>
</p>
 </li> 
 ';   
